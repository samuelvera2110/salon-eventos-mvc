...
class EventoDAO {
    ...
    public function insertar(Evento $e) {
        ...
        $stmt = $this->cn->prepare($sql);
        $stmt->execute([
            $e->nombre, $e->tipo, $e->fecha, $e->hora,
            $e->cliente, $e->salon, $e->asistentes
        ]);
    }
    ...
    public function actualizar(Evento $e) {
        ...
        nombre_evento=?, tipo_evento=?, fecha_evento=?, hora_evento=?,
        cliente=?, id_salon=?, asistentes=?
        WHERE id_evento=?";
        $stmt = $this->cn->prepare($sql);
        $stmt->execute([
            $e->nombre, $e->tipo, $e->fecha, $e->hora,
            $e->cliente, $e->salon, $e->asistentes, $e->id
        ]);
    }
